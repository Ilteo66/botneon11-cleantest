...
if (!global.groupCache) {
    global.groupCache = new NodeCache({ stdTTL: 20 * 60, useClones: false, checkperiod: 120 })
}
if (!global.jidCache) {
    global.jidCache = new NodeCache({ stdTTL: 30 * 60, useClones: false, checkperiod: 180 })
}
if (!global.nameCache) {
    global.nameCache = new NodeCache({ stdTTL: 30 * 60, useClones: false, checkperiod: 180 });
}
...
export async function participantsUpdate({ id, participants, action }) {
    ...
    try {
        ...
        if (!metadata) {
            metadata = await fetchMetadata(this, id)
            if (metadata) global.groupCache.set(id, metadata, { ttl: 600 })
        }
        if (!metadata) return
    ...
    try {
        ...
        if (m.isGroup) {
            ...
            if (groupMetadata) {
                ...
                const senderInParticipants = normalizedParticipants.find(p => p.id === normalizedSender)
                isAdmin = senderInParticipants && (senderInParticipants.admin === 'admin' || senderInParticipants.admin === 'superadmin' || senderInParticipants.isAdmin === true || senderInParticipants.admin === true)

                const botInParticipants = normalizedParticipants.find(p => p.id === normalizedBot)
                isBotAdmin = (botInParticipants && (botInParticipants.admin === 'admin' || botInParticipants.admin === 'superadmin' || botInParticipants.isAdmin === true || botInParticipants.admin === true)) || (normalizedBot === normalizedOwner || normalizedBot === normalizedOwnerLid)
