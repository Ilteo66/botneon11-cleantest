...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./${authFile}/creds.json`)) {
    do {
        ...
        const a = violet1('╭━━━━━━━━━━━━━• 𝛢𝑅𝑮𝛩 𝛣𝛩𝑇 •━━━━━━━━━━━━━');
        const b = violet1('╰━━━━━━━━━━━━━• 𝛢𝑅𝑮𝛩 𝛣𝛩𝑇 •━━━━━━━━━━━━━');
        const linea = violet2('   ✦━━━━━━✦✦━━━━━━༺༻━━━━━━༺༻━━━━━━✦✦━━━━━━✦');
...
async function connectionUpdate(update) {
    ...
    if (connection === 'close') {
        ...
        // Invia errore all'owner
        try {
            const ownerId = global.owner[0][0] + '@s.whatsapp.net';
            const errorMsg = `⚠️ *BOT DISCONNESSO*\n\n*Motivo:* ${reason || 'Sconosciuto'}\n*Connessione:* ${connection || 'Chiusa'}\n*Timestamp:* ${new Date().toLocaleString()}`;
            await global.conn.sendMessage(ownerId, { text: errorMsg });
        } catch (e) {
            console.error('Errore nell\'invio della notifica all\'owner:', e);
        }
...
process.on('uncaughtException', async (err) => {
    ...
    try {
        const ownerId = global.owner[0][0] + '@s.whatsapp.net';
        await global.conn.sendMessage(ownerId, { 
...
async function connectSubBots() {
    ...
    if (!existsSync(subBotDirectory)) {
        console.log(chalk.bold.magentaBright('🌙 𝛢𝑅𝑮𝛩 𝛣𝛩𝑇 non ha Sub-Bot collegati. Creazione directory...'));
        try {
            mkdirSync(subBotDirectory, { recursive: true });
            console.log(chalk.bold.green('✅ Directory argo-bot-sub creata con successo.'));
        } catch (err) {
            console.log(chalk.bold.red('❌ Errore nella creazione della directory argo-bot-sub:', err.message));
            return;
        }
        return;
    }
...
(async () => {
    ...
    try {
        ...
        console.log(chalk.bold.magenta(`⭑⭒━━━✦❘༻ 𝛢𝑅𝑮𝛩 𝛣𝛩𝑇 connesso correttamente ༺❘✦━━━⭒⭑`));
